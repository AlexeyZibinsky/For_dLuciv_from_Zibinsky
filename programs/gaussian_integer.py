...
class Gaussian_integer:
    ...
    def __init__( self , arg ):
        ...
        """
        if type( arg ) == complex:
            if arg.real == int( arg.real ) / 1 and arg.imag == int( arg.imag ) / 1: # Своеобразная проверка, целые ли?
    ...
    def __pow__( self , other ):
        ...
        if type( other ) == int:
            if other > 0: # Натуральная степень
                ...
                return complex( self.coefficients[0] , self.coefficients[1] ) ** other # Выходим в поле комплексных
        elif type( other ) == float:
            return complex( self.coefficients[0] , self.coefficients[1] ) ** other # Выходим в поле комплексных
        elif type( other ) == complex:
            return complex( self.coefficients[0] , self.coefficients[1] ) ** other # Выходим в поле комплексных
        elif isinstance( other , Gaussian_integer ):
            return complex( self.coefficients[0] , self.coefficients[1] ) ** complex( other.coefficients[0] , other.coefficients[1] ) # Выходим в поле комплексных
        else:
            raise ValueError('I condemn this mistake...')
    ...
    def __floordiv__( self , other ):
        ...
        if self.norm() % other.norm() != 0:
            ...
            return Gaussian_integer( self.__truediv__( other ) ) # Найденное частное
    ...
    def __rfloordiv__( self , other ):
        ...
        if other.norm() % self.norm() != 0:
            ...
            return Gaussian_integer( other.__truediv__( self ) ) # Найденное частное
